...
def main():
    ...
    print("Real-time Object + Pose + Wrist Motion Detection Started (press 'q' to quit)")
        ...
        for i, (frame, pose_res, obj_res) in enumerate(zip(frames, pose_results, obj_results)):
            ...
            if pose_res.keypoints:
                for kpts in pose_res.keypoints.xy:
                    kpts = kpts.cpu().numpy()
                    if kpts.shape[0] >= 11:
                        draw_skeleton(frame, kpts, skeleton)

                        lwrist, rwrist = tuple(map(int, kpts[9])), tuple(map(int, kpts[10]))
                        lelbow, relbow = tuple(map(int, kpts[7])), tuple(map(int, kpts[8]))
                        ...
                        if is_actively_moving(f"{cam_label}_lwrist", lwrist):
                            cv2.rectangle(frame, (lwrist[0] - lwrist_box_size, lwrist[1] - lwrist_box_size),
                                          (lwrist[0] + lwrist_box_size, lwrist[1] + lwrist_box_size), wrist_color, 2)
                        ...
                        if is_actively_moving(f"{cam_label}_rwrist", rwrist):
                            cv2.rectangle(frame, (rwrist[0] - rwrist_box_size, rwrist[1] - rwrist_box_size),
                                          (rwrist[0] + rwrist_box_size, rwrist[1] + rwrist_box_size), wrist_color, 2)
